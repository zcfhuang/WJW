...
def get_dataset(filename, pivot=0.75):
        ...
        for line in load_file(filename):
            ...
            if max_movie < int(movie):
                max_movie = int(movie)
            try:
                if rating == "1":
                    dataset[user][movie] = int(dataset[user][movie]) + 1
                if rating == "0" and int(dataset[user][movie])<1:
        ...
        for user, movies in dataset.items():
            for movie in movies:
                if random.random() < pivot:
                    ...
                    trainSet[user][movie] = dataset[user][movie]
                    trainSet_len += 1
                else:
                    testSet.setdefault(user, {})
                    try:
                        trainSet[user][movie]>-1
                        continue
                    except:
                        testSet[user][movie] = dataset[user][movie]
                    testSet_len += 1
